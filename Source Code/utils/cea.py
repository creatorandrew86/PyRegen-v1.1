...
def run_cea_SI_units(state: dict) -> list[str]:
    ...
    try: 
        ...
        engine_params["species_mass_fractions"] = c.get_SpeciesMassFractions(Pc=engine_params["Pc"], MR=engine_params["MR"], eps=nozzle_params["eps"], min_fraction=0.00001)
...
def get_station_values(state: dict, station_index: int, zone: int) -> tuple[float, float, float, float, float, list[str]]:
    errors = []
    ...
    return T_gas, T_aw, gamma, mach, Pr_gas, errors